...
module.exports = class Users {
  ...
  static changeGame(id, body, callback) {
    UsersModel.findOne({ _id: id }).then((resp) => {
      ItemsModel.findOne({ _id: body.target }).then((item) => {
        if (item) {
          let index = resp.library.indexOf(body.source);
          if (index > -1) {
            resp.library.splice(index, 1);
          }
          resp.library.push(item._id);
          resp.save().then((doc) => callback(Response.create('success', 'Jogo adicionado ao seu inventário!')));
        } else {
          callback(Response.create('error', 'Jogo não encontrado!'));
        }
      });
      if (resp) {
        resp.game = body.game;
        resp.save().then(() => callback(Response.create('success', 'Jogo alterado com sucesso!')));
      } else {
        callback(Response.create('error', 'Usuário não encontrado!'));
      }
    });
